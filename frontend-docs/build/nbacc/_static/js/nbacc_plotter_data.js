...
window.formatDataForChartJS = function(chartData) {
    ...
    function buildPointMarginData(chartData, plotType) {
        ...
        chartData.lines.forEach((line) => {
            if (plotType === "time_v_point_margin") {
                ...
                processPointMarginVsWinPercentLine(line, chartData, pointMarginData);
            }
        });
    ...
    function createChartJSConfig(
        ...
        return {
            ...
            plugins: [window.plotBackgroundPlugin, window.createWinCountPlugin(chartData)], // Add both plugins
        };
    }
    ...
    function createPluginsConfig(chartData, pointMarginData, tooltipHandler) {
        return {
            ...
            tooltip: createTooltipConfig(chartData, pointMarginData, tooltipHandler),
            zoom: window.zoomOptions,
        };
    }
    ...
    function createTitleConfig(chartData) {
        return {
            ...
            font: {
                size: isMobile() ? 12 : 18, // Smaller title on mobile
                weight: "bold",
            },
        };
    }
    ...
    function createLegendConfig() {
        return {
            ...
            labels: {
                usePointStyle: true,
                font: {
                    size: isMobile() ? 11 : 15, // Smaller on mobile
                },
        ...
        return {
            ...
            callbacks: {
                ...
                title: function (tooltipItems) {
                    return createTooltipTitle(tooltipItems, chartData, pointMarginData);
                },
            },
        };
    }
    ...
    function createTooltipTitle(tooltipItems, chartData, pointMarginData) {
        ...
        if (datasetIndex % 2 === 0 && !tooltipItems[0].chart.calculate_occurrences) {
    ...
    function createScatterTooltipTitle(
        ...
        const winPercent =
            pointData.game_count > 0
                ? ((pointData.win_count / pointData.win_plus_loss_count) * 100).toFixed(
                      2
                  )
                : "0.00";
        ...
        if (chartData.calculate_occurrences) {
            ...
            return `<div style="text-align: left;">${chartData.x_label}: ${
                ...
                (pointData.point_margin_occurs_percent * 100).toFixed(2)
            }</div>`;
        } else {
    ...
    if (chartData.lines) {
        ...
        addDatasetsToChart(chartConfig, chartData, colors, plotType);
    }
...
function generateScatterPointTooltipBody(context) {
    ...
    if (!context || !context.tooltip || !context.tooltip.dataPoints || !context.tooltip.dataPoints[0]) {
    ...
    else if (context.chart && context.chart.options && context.chart.options.chartData) {
        chartData = context.chart.options.chartData;
    }
    // If all else fails, fallback to pointMarginData
    else if (window.pointMarginData) {
    ...
    if (!chartData.lines[lineIndex].y_values || 
    ...
    // Find the matching y_values
    const pointData = chartData.lines[lineIndex].y_values.find(
        (item) => item && item.x_value === dataPoint.x && item.y_value === dataPoint.y
    );

    if (!pointData) {
        console.log("Could not find matching pointData in generateScatterPointTooltipBody");
        return "";
    }
...
window.createWinCountPlugin = function(chartData) {
    return {
        id: "winCountPlugin",

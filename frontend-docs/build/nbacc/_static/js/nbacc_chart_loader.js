...
async function loadAndPlotChart(chartDiv) {
    ...
    const jsonUrl = `${rootUrl}${staticDir}/json/charts/${
        divId.split("_copy")[0]
    }.json`;
    ...
    // Store the chart data globally BEFORE processing it
    // This ensures it's available throughout the entire chart creation process
    window.chartData = chartData;
    
    // Process JSON data for Chart.js
    const formattedData = window.formatDataForChartJS(chartData);
